...
bool replacestr(string &str, const string &str1,const string  &str2, bool bloop = false)
{
    if( (str.length() == 0) || (str1.length() == 0) ) return false;
    if((str2.find(str1) != string::npos) && (bloop == true)) return false;
    ...
    while(true)
    {
        if(bloop == true) ppos = str.find(str1);
        else ppos = str.find(str1, pstart);
...
bool replacestr(char * str, const string &str1,const string  &str2, bool bloop = false)
{
