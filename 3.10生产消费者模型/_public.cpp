...
// 如果信号量已存在，获取信号量；如果信号量不存在，则创建它并初始化value。
...
bool csemp::init(key_t key, unsigned short value, short sem_flg)
{
    ...
    // 因为信号量创建后，初始值是0，需要设置它的初始值为1，
    // 而获取信号量则不需要设置初始值。所以创建信号量和获取信号量的流程不同
...
// 信号量的P操作（把信号量的值加value(value小于0，相当于减)）,如果信号量的值是0，将阻塞等待，知道信号量的值大于0.
bool csemp::wait(short value)
{
    ...
    if (semop(m_semid,&sem_b,1) == -1) // 操作系统内核会根据 sem_b.sem_op 的值（即传入的 value）对信号量执行加减操作。
    {
        perror("p semop()");
        return false;
    }
